...
'''
the key idea is doing feature transform and uses linear hard-margin svm
solve duality question as mentioned before
to get the cofficient matirix, we have to do transform and do dot multiply
but those two steps can be merge and thus become a kernel function
'''
class kernel_hard_margin_svm:
    ...
    def __fit_bin(self, X, y):
        ...
        tmp = 0.0
        for i in range(N):
            tmp += a[i] * y[i] * self._kernel_func(X[i], X[sv_index])
        b = y[sv_index] - tmp
...
model = kernel_hard_margin_svm()
model.fit(x_train, y_train)
model.err_rate(x_test, y_test)